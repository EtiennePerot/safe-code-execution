...
class Action:
    ...
    async def action(
        ...
        emitter = EventEmitter(__event_emitter__, debug=self._debug)

        async def _fail(error_message):
            await emitter.fail(error_message)
            await emitter.code_execution_result(f"ERROR: {error_message}")
            return error_message

        await emitter.status("Checking messages for code blocks...")
        if len(body.get("messages", ())) == 0:
            return await _fail("No messages in conversation.")
        last_message = body["messages"][-1]
        if last_message["role"] != "assistant":
            return await _fail("Last message was not from the AI model.")
        split_three_backticks = last_message["content"].split("```")
        if len(split_three_backticks) < 3:
            return await _fail("Last message did not contain code blocks.")
        if len(split_three_backticks) % 2 != 1:
            return await _fail("Last message did not contain well-formed code blocks.")
        ...
        if chosen_code_block is None:
            return await _fail(
                "Last message did not contain code blocks detected as Python or Bash."
            )

        try:
            max_command_ram_bytes = None
            if self.valves.MAX_COMMAND_RAM_MEGABYTES != 0:
                max_command_ram_bytes = (
                    self.valves.MAX_COMMAND_RAM_MEGABYTES * 1024 * 1024
                )
            ...
            with tempfile.TemporaryDirectory(prefix="sandbox_") as tmp_dir:
                sandbox = Sandbox(
                    ...
                    code=code,
                    debug=self._debug,
                    networking_allowed=self.valves.NETWORKING_ALLOWED,
                    max_command_runtime_seconds=self.valves.MAX_COMMAND_RUNTIME_SECONDS,
                ...
                try:
                    ...
                    await emitter.fail(
                        f"Code timed out after {self.valves.MAX_COMMAND_RUNTIME_SECONDS} seconds"
                    )
                    status = "TIMEOUT"
                    output = e.stderr
            ...
            if status == "TIMEOUT":
                if output:
                    await emitter.message(
                        f"\n\n---\nI executed this {language_title} code and it timed out after {self.valves.MAX_COMMAND_RUNTIME_SECONDS} seconds:\n```Error\n{output}\n```\n"
                    )
